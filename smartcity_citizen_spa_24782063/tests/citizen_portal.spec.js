...
async function loginSPA(page, username, password) {
    ...
    await page.goto(`${SPA_URL}#login`, NAV_OPTIONS);
...
async function loginAdmin(page, username, password) {
    ...
    await page.goto(`${BASE_URL}/login/`, NAV_OPTIONS);
...
test.describe('Modul 1: Otorisasi & Sesi (AUTH-04, AUTH-05, AUTH-06)', () => {
    ...
    test('AUTH-04: Akses #dashboard tanpa token → redirect ke #login', async ({ page }) => {
        ...
        await page.goto(`${SPA_URL}#dashboard`, NAV_OPTIONS);
...
test.describe('Modul 5: Interaktivitas UI (UI-01 through UI-06)', () => {
    ...
    test('UI-01: Chart.js di Dashboard Admin ter-render dengan benar', async ({ page }) => {
        ...
        // -------------------------------------------------------------------
        await page.goto(`${BASE_URL}/dashboard/`, NAV_OPTIONS);
        ...
        // -------------------------------------------------------------------
        // Cara stabil untuk E2E: pastikan kedua canvas chart tersedia dan
        // memiliki ukuran render. Jika Chart.js tersedia, validasi juga instance-nya.
    ...
    //   ketik keyword pencarian di input #reportSearchInput, dan verifikasi
    ...
    test('UI-02: Live Search pada daftar laporan admin berfungsi', async ({ page }) => {
        ...
        await page.goto(`${BASE_URL}/reports/`, NAV_OPTIONS);

        // -------------------------------------------------------------------
        // LANGKAH 3: Verifikasi elemen pencarian dan tabel ada
        // -------------------------------------------------------------------
        const searchInput = page.locator('#reportSearchInput');
        const tableBody   = page.locator('#reportsTableBody');
        ...
        const responsePromise = page.waitForResponse(
            (response) => response.url().includes(`/reports/search/?q=${searchKeyword}`) && response.status() === 200,
            { timeout: 15000 }
        );
    ...
    test('UI-03: Pagination Feed Kota — maks 10 kartu, kontrol pagination muncul', async ({ page }) => {
        ...
        // -------------------------------------------------------------------
        // Setiap laporan dirender sebagai kartu .report-card di dalam #listContainer
        const listContainer = page.locator('#listContainer');
        await expect(listContainer).toBeVisible();

        const reportCards = listContainer.locator('.report-card');
    ...
    test('UI-04: Klik tombol Buat Laporan → modal #reportModal muncul', async ({ page }) => {
        ...
        // Modal awalnya belum memiliki class "show".
        await expect(reportModal).not.toHaveClass(/show/);
        await expect(reportModal).toHaveAttribute('aria-hidden', 'true');
        ...
        //
        await expect(reportModal).toHaveClass(/show/, { timeout: 5000 });
    ...
    test('UI-05: Isi form dan simpan draft → modal tutup, notifikasi muncul', async ({ page }) => {
        ...
        await page.route('**/api/reports**', async (route) => {
            ...
            if (method === 'POST') {
                ...
                // -----------------------------------------------------------
                // Mock untuk GET /api/reports/?tab=my_reports&page_size=1000
        ...
        //     Di project ini kategori menggunakan input teks.
        ...
        // -------------------------------------------------------------------
        // LANGKAH 5: Klik tombol "Simpan Draft" (#btnSaveDraft)
        // -------------------------------------------------------------------
        ...
        const reportModal = page.locator('#reportModal');
        await expect(reportModal).not.toHaveClass(/show/, { timeout: 5000 });
        await expect(reportModal).toHaveAttribute('aria-hidden', 'true');
        ...
        // Cek bahwa badge Draf menunjukkan angka > 0
        const draftBadge = summaryStats.locator('#countDraft');
